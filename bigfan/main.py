...
def identify_erroneous_inputs(variables, inputs):
    ...
    float_values = ['aif', 'site_x', 'site_y', 'cut_in', 'rated', 'ro', 'a',
                    'cut_out', 'Cp', 'availability', 'Ct', 'depth', 'yrs',
                    ...
                    'initial_num', 'constraint_scale', 'mlDenom', 'rad2',
                    'numx', 'numy', 'Lx', 'Ly']
...
def set_up_EPS(variables, values):
    ...
    values[variables.index('XLocations')] = xlocations
    values[variables.index('YLocations')] = ylocations
    ...
    output = oa.EPS(xlocations, ylocations,
                    ...
                    values[variables.index('Ct')],
                    values[variables.index('rad2')],
                    int(values[variables.index('numx')]),
                    int(values[variables.index('numy')]),
                    values[variables.index('Lx')],
                    values[variables.index('Ly')],
                    values[variables.index('mlDenom')],
...
def set_up_PSO(variables, values):
    ...
    output = oa.PSO(values[variables.index('self_weight')],
                    ...
                    int(values[variables.index('swarm_size')]),
                    int(values[variables.index('initial_num')]),
                    values[variables.index('site_x')],
                    values[variables.index('site_y')],
                    values[variables.index('turb_sep')],
...
def set_up_csv(inputfile):
    variables, values = read_inputs(inputfile)
    ...
    outobj = set_up_hardcode(variables, values, output[0], output[1],
                             values[variables.index('hh')],
                             values[variables.index('rr')], False)
